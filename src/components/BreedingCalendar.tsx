  ...
  useEffect(() => {
    const fetchCalendarItems = async () => {
      ...
      try {
        ...
        const breedingResponse = await fetch("/api/breeding");
        let breedingData: CalendarItem[] = [];
        if (breedingResponse.ok) {
          const breedingRecords = await breedingResponse.json();
          // Transform breeding records to match the calendar item format
          breedingData = breedingRecords.map((record: any) => ({
            id: `breeding-${record.id}`,
            eventName: `${record.sire.name} × ${record.dam.name}`,
            eventDate: record.startDate,
            description: `Breeding Status: ${record.status}`,
            type: "breeding",
          }));
        }
